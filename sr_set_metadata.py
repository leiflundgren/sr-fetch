...
class SrSetMetadata:
    ...
    def __init__(self, filename, overwrite, tracelevel, AtomicParsley):
        ...
        self.trace(4, 'SrSetMetadata filename=%s  overwrite=%s  tracelevel=%d  AtomicParslay="%s"' % (self.filename, self.overwrite, self.tracelevel, self.AtomicParsley) )
    ...
    def atomic_parsly_commandline(self):
        desc = self.desc
        ...
        return [ self.AtomicParsley, self.filename, '--artist', self.artist, '--album', self.progname, '--title', self.date + self.part + ' ' + desc, '--year', date ]
    ...
    def touch_creation_time(self):
        ...
        tstr = '%sT%s+0200' % (self.date, timeofday)

        dt = dateutil.parser.parse(tstr)
        new_mtime = time.mktime(dt.timetuple())
        self.trace(7, 'Setting modification time to ' + SrSetMetadata.format_datetime(dt) + ' (' + str(new_mtime) + ' seconds since 1970)')
    ...
    def main(self):
        self.parse_filename()
        cmd = self.atomic_parsly_commandline()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--atomic-parsley', help='Location of atomic-parsley', dest='AtomicParsley', default='AtomicParsley')
    ...
    for f in args.filenames:
        SrSetMetadata(f, args.overwrite, args.tracelevel, args.AtomicParsley).main()
